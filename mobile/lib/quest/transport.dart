...
abstract class QuestTransport {
  ...
  /// Дозволити/заборонити транспорту самому просити модель озвучити хід
  /// повторно, коли аудіо явно замало для тексту (обхід вади Gemini 3.1).
  /// Контролер вимикає це після кодового слова: повторне озвучення
  /// фінальної репліки означало б двічі сказане слово. Типово — увімкнено;
  /// транспорти без такого механізму просто ігнорують.
  void setRecoveryEnabled(bool enabled) {}

  /// Примусово перепід'єднатися, зберігши контекст розмови (якщо провайдер
  /// таке вміє). Контролер просить це, коли модель кілька разів поспіль не
  /// відповіла на службові сигнали — сокет міг «зависнути» без помилки.
  /// Типово — нічого не робить.

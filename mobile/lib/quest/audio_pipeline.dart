...
class AudioPipeline {
  ...
  /// Чи ще звучить голос персонажа (черга відтворення не спорожніла). Шматки
  /// аудіо приходять швидше, ніж програються, тож «останній шматок прийшов»
  /// і «персонаж договорив» — це різні моменти; для очікування відповіді
  /// людей важить саме другий.
  bool get isSpeaking => _queuedUntil > _now;

  /// Викинути все, що ще не програно з голосу персонажа (черга й буфер
  /// плеєра), і відкрити мікрофон. Використовується, коли решту ходу не
  /// треба озвучувати: персонаж почав читати ремарку-опис дії як текст, і
  /// контролер просить його повторити репліку без описів.
  void dropQueuedAgentAudio() {
    _pendingChunks.clear();

...
| 🌐 **Веб** | браузер, [`web/`](web/) | **OpenAI Realtime** (`gpt-realtime-2.1`) | редагувати персонажів і ганяти квест без заліза |
...
```
 ┌─────────┐  логін/пароль   ┌──────────────┐   ephemeral-токен   ┌──────────────┐
 │ браузер │ ───────────────▶│  наш сервер  │────────────────────▶│    OpenAI    │
 │         │                 │  (FastAPI)   │  промпт + голос     │  Realtime    │
 │  🎙 🔊  │◀═══════════ WebRTC: звук напряму, без проксі ═══════▶│ gpt-realtime │
 └─────────┘                 └──────────────┘                     └──────────────┘
                                    │
                             characters/*.yaml  ← спільні з консоллю
```

Ключ OpenAI **ніколи не потрапляє в браузер**: сервер видає короткоживучий
ephemeral-токен, а промпт і голос задає сам — клієнт їх підмінити не може.
...
├── web/                         # ВЕБ-ДОДАТОК (OpenAI Realtime)
...
│   └── static/                  #   стилі + WebRTC-клієнт (quest.js)
...
> [🌐 Веб-режим](#-веб-режим-openai-realtime)
...
Веб використовує голоси OpenAI: `marin`, `cedar` (найякісніші), `alloy`, `ash`,
`ballad`, `coral`, `echo`, `sage`, `shimmer`, `verse`. Швидкість мовлення —
поле «Швидкість» (0.25–2.0; `1.0` = звичайна).

Голос Gemini для консолі задається полем `voice` у YAML (у веб-редакторі його
навмисно немає, щоб не плутати) — тож один персонаж звучить своїм голосом у
кожному режимі.

### Мова та подача

До промпту **кожного** персонажа автоматично додаються два блоки
(див. `domovyk_quest/prompt.py`):
